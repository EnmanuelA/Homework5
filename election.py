...
def most_recent_poll_row(poll_rows, pollster, state):
    ...
    for index in range(0,len(List)-1):
        if earlier_date(List[index].get("Date"),List[index+1].get("Date")):
            List.insert(0,List[index+1])
...
def unique_column_values(rows, column_name):
    ...
    column_values = []
    for n in rows:
        column_values.append(n[column_name])

    output = set(column_values)
    return output
    pass
...
def pollster_predictions(poll_rows):
    ...
    for row in poll_rows:
        resp.append({row.get("Pollster"): {row.get("State"):(float(row.get("Dem"))-float(row.get("Rep")))}})
    return resp
    pass
